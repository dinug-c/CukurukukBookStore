...
headerComponent("Login - Cukurukuk BookStore");
...
if(isset($_POST["submit"])){
    ...
    if($valid){
        $result = getSingleMultipleBy("akun", array("Email", "Password", "Category"), array($email, md5($password), "customer"));
        if(!$result){
...
?>
<div class="flex items-center justify-center w-screen h-screen backdrop-blur-3xl bg-[#5843BE]">
        ...
        <p class="text-sm mx-auto">Belum punya akun? <a href="register.php" class="text-yellow-500 font-semibold">Daftar</a></p>
        </div>
    </div>
<?php
